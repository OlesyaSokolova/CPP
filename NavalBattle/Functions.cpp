...
namespace Sokolova {
	...
	std::unordered_map<int, calcFunc> initLastPointsMap()
	{
		std::unordered_map<int, calcFunc> result{ { FIRST_DIRECTION, firstDirection_calcLastPoint },
												  ...
												  { FOURS_DIRECTION, fourthDirection_calcLastPoint } };
		return result;
	...
	std::tuple < std::string, int, std::vector<std::string>> parsingString(const int argumentsNumber, const char ** argv)
	{
		...
		std::unordered_map <std::string, const std::string, std::hash<std::string>> PLAYERS_TYPES{ {RANDOM_KEY, RANDOM },
																							 { OPTIMAL_KEY, OPTIMAL },
																							 { USER_KEY, USER } };
	...
	void printHelp()
	{
		...
		std::cout << "game legend: x - ship element, # - ship damage, o - missed" << std::endl;
	...
	{
		std::cout << "round: " << currentRound + INDEX_SHIFT << DELIMITER_SLASH << roundsNumber << std::endl;;
	}
}
